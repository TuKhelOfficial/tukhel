...
// NEW: Import the SkeletonCard
import SkeletonCard from '../components/SkeletonCard';

export default function HomePage() {
  const { turfs } = useTurfs(); // Notice: NO MORE loading or error!

  // Start with empty array to trigger skeleton if needed
  ...
  // Shuffle turfs when they arrive from context
  useEffect(() => {
    if (turfs && turfs.length > 0) {
      setAllTurfs([...turfs].sort(() => Math.random() - 0.5));
    }
  }, [turfs]);
  ...
  // --- NEW: APP-LIKE LOADING STATE (SKELETONS) ---
  if (allTurfs.length === 0) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
        {/* Fake Hero Banner Loading */}
        <div className="w-full h-[400px] bg-gray-200 dark:bg-gray-800 animate-pulse"></div>
